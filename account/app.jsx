...
function App() {
  ...
  const [sheet, setSheet] = useState(null); // {title, sub, content}
  ...
  if (!authed) {
    const onAuth = () => { setAuthed(true); setHistory([{ screen: "home", params: {} }]); };
    if (isDesktop) {
      return (
    ...
    <div className="ac-stage">
      ...
      <div className="ac-phone" style={frame ? null : { border: "none", borderRadius: 0, boxShadow: "none", width: "100vw", height: "100dvh" }}>
        <div className="ac-app">
          ...
          <BottomNav active={activeTab} go={go} ordersBadge={ordersBadge} />
          <Toast toast={toast} />
          <Sheet open={!!sheet} onClose={closeSheet} title={sheet?.title} sub={sheet?.sub}>{sheet?.content}</Sheet>
        </div>
      </div>
